...
// Fetch user data
$user = getRecordById('users', $user_id);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            ...
            $query = "UPDATE users SET name = ?, email = ?, phone = ?, role = ?, status = ?";
            $params = [
                $user['name'],
                $user['email'],
                $user['phone'],
            ...
            // Execute the update
            $stmt = $pdo->prepare($query . " WHERE id = ?");
            $stmt->execute($params);
...
<div class="max-w-3xl mx-auto">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold text-gray-900">Edit User: <?= htmlspecialchars($user['name']) ?></h1>
                    ...
                    <div class="flex justify-between">
                        <div>
                            <?php if ($user_id != $_SESSION['user_id']): ?>
                                <a href="#" onclick="return confirmDelete(<?= $user_id ?>)" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-red-600 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500">
