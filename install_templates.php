...
// Newsletter Templates Installation Script
// This script imports the 3 professional templates into the database

require_once __DIR__ . '/includes/config.php';

echo "🍽️  Addins Meals on Wheels - Newsletter Templates Installation\n";
echo "================================================================\n\n";
...
try {
    ...
    $stmt = $pdo->query("SELECT COUNT(*) as count FROM newsletter_templates");
    $result = $stmt->fetch(PDO::FETCH_ASSOC);

    if ($result['count'] > 0) {
        echo "⚠️  Templates already exist in database!\n";
        echo "   Found {$result['count']} existing templates.\n\n";

        $response = readline("Do you want to replace them? (y/N): ");
        ...
        echo "🗑️  Removing existing templates...\n";
        $pdo->exec("DELETE FROM newsletter_templates");
        echo "✅ Existing templates removed.\n\n";
    }

    // Insert the 3 professional templates
    echo "📧 Installing professional templates...\n\n";

    // Template 1: Modern Promotional
    echo "   1️⃣  Modern Promotional Template\n";
    $stmt = $pdo->prepare("
        INSERT INTO newsletter_templates (name, description, html_template, is_active)
        VALUES (?, ?, ?, 1)
    ");
    $stmt->execute([
        'Modern Promotional',
        'Eye-catching template perfect for special offers, discounts, and promotional campaigns',
        '<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<body>
    ...
    // Template 2: Clean Announcement
    echo "   2️⃣  Clean Announcement Template\n";
    $stmt = $pdo->prepare("
        INSERT INTO newsletter_templates (name, description, html_template, is_active)
        VALUES (?, ?, ?, 1)
    ");
    $stmt->execute([
        'Clean Announcement',
        'Professional template ideal for company news, updates, and general announcements',
        '<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<body>
    ...
    // Template 3: Elegant Event
    echo "   3️⃣  Elegant Event Template\n";
    $stmt = $pdo->prepare("
        INSERT INTO newsletter_templates (name, description, html_template, is_active)
        VALUES (?, ?, ?, 1)
    ");
    $stmt->execute([
        'Elegant Event',
        'Sophisticated template perfect for special events, celebrations, and exclusive invitations',
        '<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<body>
    ...
    echo "\n✅ Professional templates installed successfully!\n";
    echo "📧 Templates available in your newsletter creation modal:\n";
    echo "   • Modern Promotional - For special offers and deals\n";
    echo "   • Clean Announcement - For news and updates\n";
    echo "   • Elegant Event - For events and celebrations\n\n";

    echo "🎯 How to use:\n";
    echo "   1. Go to Admin → Newsletters → Create Newsletter\n";
    echo "   2. Select a template from the dropdown\n";
    echo "   3. Add your content and preview\n";
    echo "   4. Send to subscribers!\n\n";

    echo "✨ Templates are fully responsive and email-client compatible!\n";

} catch (PDOException $e) {
    echo "❌ Database error: " . $e->getMessage() . "\n";
    echo "💡 Make sure your database is set up and the newsletter_templates table exists.\n";
    exit(1);
}

echo "\n🎉 Installation complete! Ready to create beautiful newsletters! 🎉\n";
?>
